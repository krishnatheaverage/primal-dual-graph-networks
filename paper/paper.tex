...
guaranteed, always valid. We also identify and fix a failure mode of the
objective: without a coverage term the size-normalized loss gives the primal
scores no incentive and they collapse, leaving the cover to a repair layer;
adding the LP coverage constraint $x_u+x_v\ge1$ as a soft penalty makes the
network learn a near-LP-optimal cover where the relaxation is tight, while
keeping the dual near-optimal and the certificate always valid. We close with the
limitations of learned certificates and the main open questions.
\end{abstract}
...
\end{enumerate}
Section~\ref{sec:exp} gives the protocol and real results, including an ablation
showing that a soft coverage term in the objective is what turns the learned
primal from a collapsed scorer into a near-LP-optimal cover, and
...
the cover cheap, keep the dual large, and keep the primal a valid cover. With
hyperparameters $\beta,\gamma>0$ the per-instance loss is the size-normalized
objective
\begin{equation}
\mathcal{L}_\theta(G,w)=\frac1n\underbrace{\sum_v w_v\,\mu_v^{(T)}}_{\text{soft primal cost}}
   \ -\ \frac{\beta}{n}\underbrace{\sum_e \hat y_e}_{\text{dual value}}
   \ +\ \frac{\gamma}{n}\underbrace{\sum_{(u,v)\in E}\mathrm{ReLU}\!\bigl(1-\mu_u^{(T)}-\mu_v^{(T)}\bigr)}_{\text{soft coverage}}. \label{eq:loss}
\end{equation}
The three terms pull in the directions a primal--dual method needs: the cost term
makes the cover cheap, the dual term keeps the certificate tight, and the
coverage term is the penalty form of the LP constraint $x_u+x_v\ge1$ on the soft
scores, which pulls $\mu$ up so the primal learns a fractional cover. The
coverage term is essential, not cosmetic: with $\gamma=0$ the objective places no
reward on the primal scores being large anywhere, so they collapse toward zero
and the integral cover is produced entirely by the repair pass; with $\gamma>0$
the cover becomes the network's own decision. We report this $\gamma=0$ versus
$\gamma>0$ contrast as a controlled ablation in Section~\ref{sec:exp}. All three
terms are $1/n$-normalized, so the loss stays in a bounded range that does not
grow with graph size (used in Section~\ref{sec:theory}); the coverage and dual
terms are each bounded $O(\Delta)$ and enter the generalization bound in the same
way. The terms are differentiable in $\theta$, and the integral rounding $\rho$
is used only at evaluation, so no discrete-gradient estimator is needed.
Feasibility of $\hat y$ and $\hat S$ comes from the architecture, not the loss.
...
on $n$ directly. Let $\Delta$ bound the maximum degree, and let node weights lie
in $[w_{\min},W]$ with $w_{\min}>0$, so instances lie in a class
$\Ical_{\Delta,W,w_{\min}}$. The lower bound $w_{\min}>0$ is needed because the
projection $\Pi_\Ycal$ divides by node loads, so with weights arbitrarily close to
zero its Lipschitz constant is unbounded; under the Uniform$[0,1]$ weights of
Section~\ref{sec:exp} one should read the bounds below with a small floor, or
replace $w_{\min}$ by the smallest realized weight.
...
Lipschitz: there is a constant
$\kappa(T,L,\Delta,w_{\min})\le C\,(L\Delta/w_{\min})^T$ for an absolute constant
$C$, such that for parameter vectors $\theta,\theta'$,
\[
...
(c) the projection $\Pi_\Ycal$, which contributes a Lipschitz factor
$O(1/w_{\min})$ because it multiplies its inputs by scalars
$\min_{v\in e}\lambda_v\in[0,1]$, and $\lambda_v=\min\{1,w_v/\ell_v\}$ has slope up
to $1/w_v\le 1/w_{\min}$ in the loads. Composing $T$ such rounds multiplies the
per-round constant $O(L\Delta/w_{\min})$, giving $\kappa=O((L\Delta/w_{\min})^T)$.
...
and $P$ coordinates, weights in $[w_{\min},W]$, and degree bounded by $\Delta$. The
size-normalized loss \eqref{eq:loss} takes values in an interval of width
$M=O(1+(\beta+\gamma)\Delta)$. For a sample of $N$ i.i.d.\ instances from $\Dcal$, with
...
$\kappa(T,L,\Delta,w_{\min})=O((L\Delta/w_{\min})^T)$ is the Lipschitz constant of
Lemma~\ref{lem:stab}.
\end{theorem}
\begin{proof}[Proof sketch]
By Lemma~\ref{lem:stab} and the $1/n$ normalization in \eqref{eq:loss}, the loss
is $O((W+(\beta+\gamma)\Delta)\kappa)$-Lipschitz in $\theta$: the per-node average
$\frac1n\sum_v w_v\mu_v$ has Lipschitz constant at most $W\kappa$ because the
$1/n$ cancels the $n$ summed node terms, and the dual and coverage averages each
contribute at most $O((\beta+\gamma)\Delta\kappa)$ since a bounded-degree graph
has $O(n\Delta)$ edges and the coverage hinge is $1$-Lipschitz in $\mu$. An
$\epsilon$-net of the radius-$R$ ball in $\real^P$ induces an
$O((W+(\beta+\gamma)\Delta)\kappa\epsilon)$-cover of the induced function class, and the
...
On instance families with $\Delta=O(1)$, $w_{\min}=\Omega(1)$, and fixed
$\beta,\gamma$, the sample complexity to reach excess risk $\epsilon$ is
$\tilde O\!\bigl(P\,(T\log(L\Delta/w_{\min}))/\epsilon^2\bigr)$, with no polynomial
dependence on the number of nodes $n$.
\end{corollary}
...
optimizer can find. The factor $(L\Delta/w_{\min})^T$ is the usual cost of depth in bounds
...
the certificate at $O(\log n)$. A subtler limitation shows up in the primal once
the objective is right. The coverage term of \eqref{eq:loss} fixes the collapse
that the naive $\gamma=0$ objective produces (Section~\ref{sec:exp}): with
$\gamma>0$ the network learns a soft fractional cover and the integral solution
becomes its own decision rather than the repair layer's. But what it learns is
essentially the LP relaxation, so the quality of the rounded cover tracks the LP
integrality gap: near-optimal where the relaxation is tight, and loose, with
high-variance threshold rounding, on families like dense Erd\H{o}s--R\'enyi where
the LP is highly fractional, the same half-integrality that hurts LP rounding.
Pushing the learned primal to be decisive (away from $\mu=\tfrac12$) so that it
beats LP rounding on the hard half-integral instances too is open. We
deliberately avoid a supervised primal target as the main fix, since learning
from optimal solutions is exactly the lever a separate line of
work~\citep{he2025primal} uses to surpass the algorithm, and an unsupervised,
constraint-based primal is part of what distinguishes the certificate framing
here. The generalization bound carries the usual
